...
def apply_umap_with_classifier(final_df):
    import umap
    from sklearn.preprocessing import StandardScaler
    ...
    # Label mapping for numerical encoding
    label_mapping = {'control': 0, 'mixture': 1, 'mix': 1, 'sulfamethoxazole': 2, 'sulfapyridine': 3, 'sulfathiazole': 4}
    ...
    # Apply UMAP on the training set
    umap_transformer = umap.UMAP(n_components=128, random_state=42, n_neighbors=100)
    X_train_umap = umap_transformer.fit_transform(X_train_scaled, y_train)
    X_test_umap = umap_transformer.transform(X_test_scaled)
    # Convert data to PyTorch tensors
    X_train_tensor = torch.tensor(X_train_umap, dtype=torch.float32)
    y_train_tensor = torch.tensor(y_train.values, dtype=torch.long)
    X_test_tensor = torch.tensor(X_test_umap, dtype=torch.float32)
    ...
    return umap_transformer, model

def apply_mlp(final_df):
    from sklearn.model_selection import train_test_split, StratifiedKFold
    from sklearn.metrics import classification_report
    import pandas as pd
    import numpy as np
    ...
    # Define a simple MLP model
    class MLPClassifier(nn.Module):
        def __init__(self, input_dim=X_train_tensor.shape[1], num_classes=5):
            super(MLPClassifier, self).__init__()
            self.model = nn.Sequential(
    ...
    # Define loss and optimizer
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=0.0002, betas=(0.7, 0.9)) #0.0005
    # Training loop
    epochs = 25
    model.train()
    for epoch in range(epochs):
    ...
    # Class names in the correct label order
    class_names = [
        'control',
        'mixture',
        'sulfamethoxazole',
...
def split_data(image_paths, labels, test_size=0.2, random_state=42):
    ...
    train_dir = 'C:/Users/diogo/Desktop/radar/dataset/train'
    test_dir = 'C:/Users/diogo/Desktop/radar/dataset/test'
    if not os.path.exists(train_dir):
        os.makedirs(train_dir)
    if not os.path.exists(test_dir):
        os.makedirs(test_dir)
    # Copy the images to their respective train/test directories
...
'''def exploratory_ml(df, group_val):
    from sklearn.model_selection import train_test_split
    from sklearn.utils import shuffle
    from lazypredict.Supervised import LazyClassifier
    import seaborn as sns
    ...
    models_test, predictions_test = clf.fit(X_train, X_test, y_train, y_test)
    models_test['Accuracy (%)'] = models_test['Accuracy'] * 100
    # Save Excel if folder specified
    models_test.to_excel(os.path.join("C:/Users/diogo/Desktop/Bernardo_ox/results/", f"{group_val}models.xlsx"))'''
